...
class VariationalAutoEncoder(nn.Module):
    def __init__(self, encoder, decoder, conditional=False, path_to_model='vae_model/', device='cpu', n_epoch=10000,
    ...
    def forward(self, x, condition=None):
        if self.cvae:
            mu, logstd = self.encoder(x, condition)
        else:
    ...
    def forward_deploy(self, x, condition=None):
        if self.cvae:
            mu, logstd = self.encoder(x, condition)
        else:
            mu, logstd = self.encoder(x)
        ...
        z = eps.mul(var).add(mu)
        if self.cvae:
            xhat = self.decoder(mu, condition)
        else:
    ...
    def update_beta(self):
        ...
        if self.epoch > epoch_to_update:
            if self.average_kl_loss > self.target_kl_loss:
                self.beta = (self.beta_idx + 1.0) / self.beta_steps * (self.beta_max - self.beta_min) + self.beta_min
                self.beta_idx += 1
    ...
    def train_vae(self, data_loader):
        ...
        for self.epoch in range(self.n_epoch):
            ...
            for x, c in data_loader:
                ...
                optimizer.zero_grad()
                xhat, mu, logsd, z = self.forward(x, c)
    ...
    def evaluate(self, data_loader):
        ...
        for i, data in enumerate(data_loader):
            ...
            if isinstance(x, list):
                ...
                x = x[0].to(self.device)
            xhat, mu, logsd, z = self.forward_deploy(x, c)
    ...
    def decode(self, z, c):
        ...
        if self.cvae:
            c = c.float()
            xhat = self.decoder(z, c).detach().numpy()
        else:
    ...
    def save_model(self):
        ...
        for i in range(3):
            plt.subplot(3, 1, i + 1)
            plt.plot(np.arange(self.snapshot - 1), self.hist_losses[self.epoch - self.snapshot + 1:self.epoch, i],
                     label=plt_labels[i])
