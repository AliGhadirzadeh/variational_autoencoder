...
class FullyConnectedConditionalEncoder(nn.Module):
    def __init__(self, input_size, condition_size, output_size):
        super(FullyConnectedConditionalEncoder, self).__init__()
    ...
    def forward(self, data, condition):
        ...
        x = torch.cat((data, condition), axis=1)
...
class FullyConnectedConditionalDecoder(nn.Module):
    def __init__(self, latent_size, condition_size, output_size):
        super(FullyConnectedConditionalDecoder, self).__init__()
        self.input_size = latent_size
        self.output_size = output_size
        self.fc1 = nn.Linear(latent_size + condition_size, 128)
        self.fc2 = nn.Linear(128, 256)
        self.fc3 = nn.Linear(256, 512)
        self.fc4 = nn.Linear(512, output_size)
        self.bn1 = nn.BatchNorm1d(128)
    ...
    def forward(self, latent, condition):
        x = torch.cat((latent, condition), axis=1)
        x = F.relu(self.bn1(self.fc1(x)))
        x = F.relu(self.bn2(self.fc2(x)))
        x = F.relu(self.bn3(self.fc3(x)))
        x = self.fc4(x)
